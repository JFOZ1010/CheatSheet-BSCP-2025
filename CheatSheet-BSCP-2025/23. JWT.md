...
```
 * Tomamos la secret1 la codeamos a base64 y luego la añadimos en nuestra clave generada en la clave que es “k”: ahi añadimos el valor de secret1 a base64
...
-> Generate RSA key in extension //va dentro del header del JWT
-> In repeater jwt tab change value to administrator, then select attack with embedded JWK
//Ya luego simplemente actualizar el endpoint a /admin o delete carlos. 
```
...
and paste it on exploit server inside la key generada `copy as a JWK` dentro de JWT Token Editor, el header de JWT toma la url de nuestro server exploit que allá tendremos alojado nuestros valores de nuestra RSA Key generados y lo validará como legitimo
 -> dentro de estas llaves debemos alojar la public key RSA JWK: { "keys" : []} 
...
{  
    "kid": "265576ea-fc45-42c8-a21c-5a921c4f37c5", 
    "alg": "RS256",  
    "jku": "https://exploit-ID-EXPLOIT.net/exploit"
}
}]}
