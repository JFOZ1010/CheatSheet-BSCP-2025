...
- Para este caso `GET /post?postID=ID` RENAME > **DESENCRYPT**
...
2. Hacemos lo mismo copiamos la cookie y vamos a decoder desencoded>url>base64 y eliminamos 32 bytes, luego encoded>base64>url y esa `cookie` la actualizamos en la cookie de `GET /post?postId=ID` y veremos reflejado el user administrator 
3. Con eso sabemos que nuestra `cookie notification` sirve, tomamos la `GET / HTTP/2` y actualizamos la cookie notification con el valor de nuestra cookie `session` desde la extension de **cookie editor** y eliminamos el valor de la cookie `session anterior` luego en la response veremos reflejado el `admin panel` eso es todo, fin. 
...
Este es un laboratorio engorroso, toca crear un intruder con el endpoint `POST post/cart` del la chaqueta de cuero y le dejamos en quantity `99` luego un payload de tipo null payload con exactamente `323` null payloads, luego de ello vamos a repeater y mandamos en el mismo tipo `POST post/cart` pero esta vez una quantity de `47` y vemos que se ve un precio reflejado muy bajo, listo ahora es cuestión de tomar otro producto añadirlo `preferiblemente de un precio bajo` e ir añadiendo nuevas unidades 1 a 1 para ver como el precio global que estaba en negativo empieza a bajar hasta llegar a una unidad positivas pero muy baja ejm: $10.00
